""" feature_axis.py: IDENTIFY NORMALIZED AND ORTHOGONAL FEATURE AXES IN THE LATENT SPACE """
import numpy as np
import pandas as pd
import dnnlib
import config
...
def gram_schmidt(vectors):
    # convert feature directions into normalized and orthogonal feature axes,
    # columns in vectors correspond to feature directions
    basis = []
...
def main():
    """ Step 1: prepare for the training data: (dlatent_array, feature_array) """
    ...
    for feature_label in feature_label_list:
        if feature_label == 'gender':
            ...
            feature_dict[feature_label] = np.array([x['faceAttributes'][feature_label]['eyeMakeup']
                                                    for x in labels_data]) \
                                          + np.array([x['faceAttributes'][feature_label]['lipMakeup']
                                                      for x in labels_data])
        elif feature_label == 'glasses':
            feature_dict[feature_label] = np.array([x['faceAttributes']['glasses'] == 'ReadingGlasses'
                                                    for x in labels_data]) \
                                          + np.array([x['faceAttributes']['glasses'] == 'Sunglasses'
                                                      for x in labels_data]) \
                                          + np.array([x['faceAttributes']['glasses'] == 'SwimmingGoggles'
                                                      for x in labels_data])
        else:
...
#       feature_dict[feature_label] = np.array([x['faceAttributes']['facialHair'][feature_label] for x in labels_data])
    ...
    for feature_label in feature_label_list_haircolor:
        ...
        for i in range(num_example):
            ...
            if tempt:
                feature_dict[feature_label+'Hair'][i] = [x['confidence'] for x in tempt
                                                         if x['color'] == feature_label][0] >= 0.95

    feature_DataFrame = pd.DataFrame(feature_dict)
    feature_array = np.array(feature_DataFrame, dtype = float)
    feature_array = (feature_array - np.min(feature_array, axis=0))/(np.max(feature_array, axis=0)
    ...
    """ Step 3: normalize and orthogonalize feature_directions to get feature axes """
