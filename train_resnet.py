...
from this thread: https://github.com/Puzer/stylegan-encoder/issues/1#issuecomment-490469454
"""
import os
import math
import numpy as np
...
def generate_dataset_main(n=10000, seed=None, model_res=1024, image_size=256, minibatch_size=16, truncation=0.7):
    """
...
def generate_dataset(n=10000, save_path=None, seed=None, model_res=1024, image_size=256,
                     minibatch_size=16, truncation=0.7):
    """
    ...
    W, X = generate_dataset_main(inc, seed, model_res, image_size, minibatch_size, truncation)
    for i in range(batch_size-2):
        aW, aX = generate_dataset_main(inc, seed, model_res, image_size, minibatch_size, truncation)
        W = np.append(W, aW, axis=0)
        X = np.append(X, aX, axis=0)
    aW, aX = generate_dataset_main(left, seed, model_res, image_size, minibatch_size, truncation)
    W = np.append(W, aW, axis=0)
    X = np.append(X, aX, axis=0)
...
def finetune_resnet(model, model_res=1024, image_size=256, batch_size=10000, test_size=1000, n_epochs=10,
                    max_patience=5, seed=0, minibatch_size=32, truncation=0.7):
    """
    Fine tunes a resnet to predict W from X
    ...
    while patience <= max_patience:
        W_train, X_train = generate_dataset(n=batch_size, model_res=model_res, image_size=image_size, seed=seed,
                                            minibatch_size=minibatch_size, truncation=truncation)
...
parser = argparse.ArgumentParser(description='Train a ResNet to predict latent representations of images in a StyleGAN '
                                             'model from generated examples',
...
parser.add_argument('--loop', default=1, help='Run this many iterations', type=int)
...
if args.freeze_first:
    ...
    finetune_resnet(model, model_res=args.model_res, image_size=args.image_size,
                    batch_size=args.batch_size, test_size=args.test_size, max_patience=args.max_patience,
                    n_epochs=args.epochs, seed=args.seed, minibatch_size=args.minibatch_size, truncation=args.truncation)
