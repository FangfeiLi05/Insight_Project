...
def find_feature_axis(z, y, method='linear', **kwargs_model):
    ...
    if method == 'linear':
        ...
        def sigmoid_r_clip(y):  
            tempt = np.clip(y, 0+1e-3, 1-1e-3)
            tempt = np.log(tempt) - np.log(1-tempt)
            return tempt
        
        model = linear_model.LinearRegression(**kwargs_model)
        model.fit(z, sigmoid_r_clip(y))
        
    else:
        raise Exception('method has to be one of ["linear", "tanh", "sigmoid"]')
